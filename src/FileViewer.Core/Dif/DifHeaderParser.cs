        ...
        ParseHeaderAndFields(ReadOnlySpan<byte> headWindow, List<DifDiagnostic> diagnostics)
    {
            ...
            || !DifLineScanner.LineEqualsMarker(firstLine, DifFormatOptions.HeaderStart))
        {
            diagnostics.Add(new DifDiagnostic(DifDiagnosticSeverity.Error,
                $"File does not start with '{DifFormatOptions.HeaderStart}'.", 0));
            return null;
        }
        ...
        if (!DifLineScanner.TryReadLine(headWindow, ref pos, out ReadOnlySpan<byte> dataStartLine)
            || !DifLineScanner.LineEqualsMarker(dataStartLine, DifFormatOptions.DataStart))
        {
            diagnostics.Add(new DifDiagnostic(DifDiagnosticSeverity.Error,
